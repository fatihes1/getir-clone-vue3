            ...
            <vue-collapsible-panel-group accordion base-color="#ffffff">
                <ul class="grid md:gap-y-2">
                  <vue-collapsible-panel :expanded="false">
                    ...
                    <template #content>
                        <li v-for="item in data.items" :key="item" class="mb-3">
                          <a class="text-sm  text-gray-600 hover:text-primary-brand-color/90" :href=item.url>{{ item.subTitle }}</a>
                        </li>
                    </template>
...
.vcp__header-icon {
  height: 22px;
  width: 22px;
  background-color: rgba(93,62, 188 , 0.1);
  border-radius: 25%;
  color: #5d3ebc;
}
</style>